...
// ┌─────────────────────────────────────────────────────────────────────────┐
// │  核心总结：tx 的归宿决定是否需要手动 drop                                     │
// ├──────────┬──────────────────────────────────┬───────────────────────────┤
// │          │ tx 的归宿                         │ 是否需要手动 drop           │
// ├──────────┼──────────────────────────────────┼───────────────────────────┤
// │ demo_1   │ 留在外层 async 块作用域             │  需要，否则 while 循环挂起   │
// │ demo_2   │ async move 移入 tx_fut            │ 不需要，Future 结束自动drop  │
// │ demo_3   │ tx → tx_fut；tx.clone() → tx1_fut │ 不需要，各 Future 结束时     │
// │          │ （两个 Sender 均通过 move 转移）    │ 自动 drop 各自的 Sender     │
// └──────────┴──────────────────────────────────┴───────────────────────────┘
//
// 规则：channel 在**所有 Sender 全部 drop** 后关闭，rx.recv() 才返回 None。
// 只要 tx 通过 `async move` 转移进 Future，Future 执行完毕即自动 drop，
// 无需显式调用 drop(tx)。
fn main() {
    demo_1();
